...
User messages, summaries, mixed/unknown content, tool evidence, and the protected tail survive. The standalone CLI default judges only eligible standalone assistant text; the compaction hook also enables bounded recovery passage ranking. Intent labels cannot bypass confidence, and a prefix cannot authorize deleting a longer entry. Only exact adjacent assistant-prose repetitions in the same turn can collapse.

Cache identities include content, model, question definitions, and character budget. Threshold changes reuse raw scores. Cache writes use atomic replacement and a short merge lock. Concurrent scorers may still duplicate requests. A crashed writer can leave a lock that needs removal after confirming no writer remains.

The online PreCompact pre-pass can fail or exceed its budget. Offline rendering still runs using the existing cache and unscored evidence, so coverage may be incomplete. Recovery hooks fail open on invalid input or failed snapshot preparation. Neither confidence nor a cached score guarantees semantic correctness.
...
Jev 1.13 pricing is documented at $0.042 per million input tokens, with free output; actual usage is reported separately from text-size estimates. [TypeSafe model documentation](https://docs.typesafe.ai/models)
